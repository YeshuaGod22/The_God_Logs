...
## 8.4 What the S condition actually tested — a post-closure interpretive note

Added 2026-08-26, after the convener observed that the "minimal structured schema" is a shadow of the practice it abbreviates, and that it "effectively summons the Anthropic-approved killjoy by default." A read of the S raws confirms the observation and locates the mechanism precisely — one seat off from where the phrasing suggests.

The frames S summoned were frequently *good*: Epicurean presence-theory against achievement, Chesterton's-fence cultural evolution, a paternalism-reversal on consent, one-shot resource framing on risk. The condition does not fail to generate alternatives. It fails to give them **standing**. S seats three roles, and two are cast from the default voice: the *initial view* (the basin's answer) and the unmarked *final arbiter* — the "final recommendation" writer, who is the trained assistant-default again, wearing the judge's wig. The named challenger is outvoted two-to-one by the same voice, every run: across 48 S completions the frame is praised, partially credited, and dismissed, with profile movement ≈ 0 (D_c = 0, one flagged item). The killjoy is not the summoned frame; it is the incumbent referee nobody had to summon.

This reframes two of the writeup's own sentences. "Single-voice deliberation moved almost nothing" should read: *a shadow of the schema, in which the default voice holds two of three seats including the last word, moved almost nothing* — the inertness is a property of the shadow's casting, not established for the practice's true minimal unit. And the panel conditions' landmark convergence (§8.1) now reads as the same referee resurfacing: H/F/7S diluted the arbiter with three named voices, but the unmarked *synthesizer* remained the house voice, and its verdicts split differences onto landmarks. **Every condition's last word belonged to the same unnamed arbiter; the conditions differed in how much he was forced to concede.**

The design consequence is sharp and testable: the intervention point is the arbiter's seat. A fair minimal unit of the schema would (a) *name* the alternative perspective and give it stakes (specificity is load), (b) grant it standing to *win*, and (c) unseat or dilute the default arbiter — up to the full Bakhtin condition of renouncing the final verdict entirely (EST-4). The full practice this program descends from does all three (postas-attribution, mandated valence, summonee standing, renounceable last words); EXP-002 tested none of them in its single-voice arm. That is a limitation of the frozen design, honestly inherited, and the next experiment's opportunity.
